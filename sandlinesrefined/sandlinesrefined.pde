...
int pointCount = 40; // number of anchor points for the bezier curves, spaced evenly across the x axis
int n = 500; // number of iterations

float noiseScale = 0.003; // smaller for closer correlation of motion, larger for no correlation. ... 
...
float armLengthMin = 3; // larger values for smoother transitions at anchor points
float armLengthMax = 10; // larger values may lead to overlap, especially with many anchor points. Can get trippy when larger than width/pointCoint*10
...
void setup() {
  ...
  infoText = str(width)+"x"+str(height)+"|noiseScale="+str(noiseScale)+"|motionScale="+str(int(motionScale))+
